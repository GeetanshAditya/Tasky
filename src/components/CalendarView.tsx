...
export default function CalendarView() {
  ...
  return (
    <div className="h-full flex flex-col">
      {/* Header */}
      <div className="p-4 md:p-6 border-b border-gray-700/50 bg-gray-800/20 backdrop-blur-sm">
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-4">
          <div className="mb-4 md:mb-0">
            <h1 className="text-xl md:text-2xl font-bold text-white">Calendar</h1>
            <p className="text-gray-400 mt-1 text-sm md:text-base">View completed tasks by date</p>
          </div>
          <button
            onClick={() => dispatch({ type: 'SET_VIEW_MODE', payload: 'tasks' })}
            className="flex items-center space-x-2 px-4 py-2 bg-purple-600/20 hover:bg-purple-600/30 border border-purple-500/30 rounded-lg transition-colors w-full md:w-auto justify-center"
          >
            <List className="w-4 h-4" />
            <span className="text-sm md:text-base">Task View</span>
          </button>
        </div>
      ...
      <div className="flex-1 p-4 md:p-6">
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-4 md:gap-6 h-full">
          ...
          <div className="xl:col-span-2">
            <div className="bg-gray-800/30 backdrop-blur-sm rounded-xl p-4 md:p-6 border border-gray-600/50 h-full">
              {/* Day Headers */}
                ...
                {calendarDays.map(day => {
                  ...
                  return (
                    <button
                      ...
                      className={`
                        relative p-2 md:p-3 rounded-lg border transition-all hover:bg-gray-700/50
                        ${isSelected 
              ...
              {state.selectedDate ? (
                selectedDateTasks.length > 0 ? (
                  ...
                  <div className="text-center py-8">
                    <div className="w-10 h-10 md:w-12 md:h-12 bg-gray-700/30 rounded-full flex items-center justify-center mx-auto mb-3">
                      <CheckCircle className="w-5 h-5 md:w-6 md:h-6 text-gray-400" />
                    </div>
                ...
                <div className="text-center py-8">
                  <div className="w-10 h-10 md:w-12 md:h-12 bg-gray-700/30 rounded-full flex items-center justify-center mx-auto mb-3">
                    <CalendarIcon className="w-5 h-5 md:w-6 md:h-6 text-gray-400" />
                  </div>
                  <p className="text-gray-400 text-xs md:text-sm">
